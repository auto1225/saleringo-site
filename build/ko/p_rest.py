...
verified = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-grid', 'life', '답변 하나가 만들어지기까지',
     ...
     '대신 나갑니다. 막히는 쪽이 기본값입니다 &mdash; 애매하면 답하지 않습니다.',
     pipe(['질문 이해', '등록된 자료에서 찾기', '금지 규칙 검사', '답변 또는 사람에게'])
     + ul([('질문 이해',
            '무엇을 묻는지 정리합니다. 여러 개를 한꺼번에 물으셨으면 나눕니다. '
            '&ldquo;임플란트 얼마고 이번 주에 되나요&rdquo;는 금액 질문 하나와 '
            '일정 질문 하나입니다.'),
           ('등록된 자료에서 찾기',
            '사장님이 넣으신 요금표·서비스 목록·영업시간에서만 찾습니다. 인터넷에서 '
            '찾지 않고, 모델이 알고 있는 일반 지식으로 메우지도 않습니다. '
            '<b>여기서 못 찾으면 그 자리에서 멈춥니다.</b>'),
           ('금지 규칙 검사',
            '찾았더라도 말하면 안 되는 것이 있습니다. 진단 비슷한 말, 확정적인 '
            '치료 결과, 요금표에 없는 할인. 업종마다 목록이 다르고, 그 목록을 '
            '사장님이 보고 고치실 수 있습니다.'),
           ('답변 또는 사람에게',
            '통과하면 답하고, 막히면 담당자 앞으로 건이 넘어갑니다. 넘어간 건은 '
            '수신함에 남으므로 끊고 나서 잊히지 않습니다.')])),
 sec('t-md sec-dark bg-spot', 'refuse', '하지 않는 일',
     '기능을 끄는 것이 아니라<br>처음부터 막아 둡니다.', '',
     ul([('없는 것을 만들지 않습니다.',
          '등록된 요금표와 서비스 목록에 없으면 답하지 않습니다. 비슷한 것으로 대신하지도 '
          '않습니다. &ldquo;대략 이 정도&rdquo;가 가장 위험합니다 &mdash; 그 금액을 '
          '들은 손님은 그 금액을 기억하고 오십니다.'),
         ('판단하지 않습니다.',
          '진단, 법률 판단, 사건 전망, 합격 가능성 같은 것은 사람만 합니다. '
          '「이 증상이면 아마…」로 시작하는 문장은 만들어지지 않습니다. '
          '자격이 있는 사람이 해야 하는 말이기 때문입니다.'),
         ('사람인 척하지 않습니다.',
          '물어보시면 AI라고 답합니다. 숨기면 나중에 알게 되고, 나중에 알게 되는 것이 '
          '처음부터 아는 것보다 훨씬 나쁩니다.'),
         ('없는 시간에 예약을 잡지 않습니다.',
          '캘린더의 빈 시간에서만 잡습니다. 중복 예약이 나면 오신 손님을 돌려보내야 하고, '
          '그건 전화를 못 받은 것보다 나쁩니다.'),
         ('안전이 걸리면 붙잡지 않습니다.',
          '흉통, 가스 냄새, 누수, 아이가 다쳤다는 말이 나오면 예약을 잡지 않고 즉시 '
          '사람에게 돌립니다. 그 상황에서 예약 시간을 안내하는 것은 도움이 아닙니다.'),
         ('돈을 확정하지 않습니다.',
          '계약금, 환불, 할인 예외처럼 금액이 확정되는 건은 사람이 승인합니다. '
          'AI 는 요금표에 적힌 것을 안내할 뿐 예외를 만들지 않습니다.')])),
 sec('t-md sec-light bg-paper', 'audit', '감사 기록',
     '왜 그렇게 답했는지가<br>전부 남습니다.', '',
     ul([('답변마다 근거가 붙습니다.',
          '요금표의 어느 항목을 보고 그 금액을 말했는지 기록됩니다. 나중에 '
          '&ldquo;그때 이렇게 들었는데요&rdquo; 하는 상황이 오면, 실제로 무엇을 '
          '안내했고 무엇을 근거로 했는지를 열어 보실 수 있습니다.'),
         ('막힌 것도 기록됩니다.',
          '무엇을 물었는데 왜 답하지 않았는지가 남습니다. 이것이 가장 쓸모 있는 '
          '자료입니다 &mdash; 자주 막히는 질문이 곧 요금표에 빠진 항목이고, '
          '그것을 채우면 다음 달에는 답합니다.'),
         ('내려받을 수 있습니다.',
          '기록은 제품의 일부입니다. 요청해야 주는 것이 아니고, 해지하실 때만 '
          '주는 것도 아닙니다. 쓰시는 동안 아무 때나 전부 받으십니다.'),
         ('저희가 임의로 열어 보지 않습니다.',
          '장애 조사처럼 필요한 경우에만, 사유를 남기고 봅니다. 그 접근 기록은 '
          '요청하시면 보여 드립니다. &ldquo;안 본다&rdquo;고 말하는 것보다 '
          '&ldquo;보면 기록이 남는다&rdquo;가 확인 가능한 약속입니다.')]),
     dark=False),
...
about = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-grid', 'why', '왜 이걸 만들었는가',
     '세 가지가 동시에 틀리면<br>손님이 다음 가게로 갑니다.',
     '전화를 못 받고, 못 받은 것을 모르고, 알았을 때는 이미 늦었습니다. 이 셋이 겹치는 '
     '순간이 하루에도 여러 번 있고, 그때마다 손님 한 분이 조용히 사라집니다. 이 손실은 '
     '어떤 장부에도 안 잡히기 때문에, 줄이려는 시도조차 하지 않게 됩니다.',
     trio([('받아 주는 서비스는 이미 있었습니다',
            '그런데 결과물이 메모 한 줄이라, 다음 날 그 메모를 보고 다시 걸어야 했습니다. '
            '통화 한 건이 두 건이 됩니다.'),
           ('챗봇도 이미 있었습니다',
            '정해 둔 답을 고르게 하고 마지막에 「상담 신청」을 누르게 합니다. '
            '그러면 또 사람이 전화를 걸어야 합니다.'),
           ('그래서 끝까지 가게 만들었습니다',
            '받고, 답하고, 예약을 잡고, 기록으로 남기는 데까지. 중간에 사람이 다시 '
            '개입해야 하면 그건 절반만 한 것입니다.')])),

 sec('t-md sec-dark bg-spot', 'promise', '약속하는 것',
     '지킬 수 있는 것만<br>적습니다.', '',
     ul([('데이터는 그 가게의 것입니다.', '언제든 전부 내려받을 수 있고, 해지할 때 막지 않습니다.'),
         ('다른 고객 데이터로 학습하지 않습니다.', '대화 내용을 다른 고객의 AI 학습에 쓰는 일은 절대 없습니다. '
...
guide = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-dusk', 'diff', '셋이 어떻게 다른가',
     ...
     '그리고 무엇이 남느냐가 곧 사장님 일이 줄어드느냐를 정합니다.',
...
cross = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-grid', 'time', '시차',
     ...
     '이쪽이 문을 닫은 시간입니다. 다음 날 아침에 답장을 보내면 그때는 그쪽이 퇴근한 뒤입니다. '
     '이렇게 하루에 한 번씩 주고받으면 견적 하나에 영업일 사흘이 걸립니다.',
 ...
 sec('t-md sec-dark bg-dusk', 'who', '어느 쪽이신가',
     '방향이 둘입니다.<br>필요한 것이 다릅니다.',
     '한국에서 해외로 파는 회사와, 한국에 오는 외국인 손님을 받는 가게는 같은 문제를 '
     '반대 방향에서 겪습니다. 무엇이 필요한지도 다릅니다.',
     trio([('해외로 파는 회사',
            '문의가 우리 새벽에 옵니다. 아침에 답하면 그쪽은 이미 퇴근했고, 하루에 '
            '한 번씩만 주고받게 됩니다. 견적 하나에 영업일 사흘이 걸립니다.'),
           ('한국에 오는 손님을 받는 가게',
            '예약 문의가 영어·중국어·일본어로 옵니다. 번역기로 답하면 뜻은 통하는데 '
            '신뢰가 안 생깁니다. 그리고 시간대 표기에서 사고가 납니다.'),
           ('둘 다인 경우',
            '수출도 하고 방문 손님도 받는 곳이 많습니다. 같은 수신함에서 언어만 '
            '갈라 처리합니다.')])),

 sec('t-md sec-dark bg-grid', 'tz', '시간 표기',
     '「내일 오후 3시」가<br>어느 나라 3시인지.',
     '해외 예약에서 가장 자주 나는 사고입니다. 확인 메일에 시간 하나만 적혀 있으면 '
     '받는 사람은 자기 시간으로 읽습니다. 그러면 하루를 잘못 알고 오거나, 안 옵니다.',
     ul([('두 시간대를 같이 적습니다',
          '확인 문자와 메일에 현지 시간과 한국 시간을 함께 적습니다. '
          '「3월 4일 오후 3시(KST) · 3월 4일 오전 1시(EST)」처럼요.'),
         ('서머타임을 계산합니다',
          '미국과 유럽은 해마다 시간이 바뀝니다. 그 주에 잡힌 예약이 한 시간씩 어긋나는 '
          '일이 실제로 자주 납니다. 예약 시점의 규칙으로 계산합니다.'),
         ('상대 근무시간을 봅니다',
          '그쪽 업무시간 밖으로 잡지 않습니다. 우리 오전이 그쪽 새벽이면 그 시간은 '
          '제시하지 않습니다.')])),
...
demo = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-spot', 'callnow', '지금 걸어 보십시오',
     ...
     '&mdash; 막혔을 때 어떻게 하는지가 오히려 더 잘 보입니다.',
     trio([('예약을 잡아 보세요',
 ...
 sec('t-md sec-dark bg-spot', 'howread', '이 통화를 어떻게 읽으시면 되는가',
     '세 지점만 보시면 됩니다.',
     '통화 전체를 읽으실 필요는 없습니다. 아래 세 곳에서 이 제품이 다른 것들과 갈립니다.',
     trio([('금액이 어디서 나왔는가',
            '숫자가 나오는 자리마다 그 근거가 요금표의 어느 줄인지 붙어 있습니다. '
            '근거가 없는 숫자는 나오지 않습니다.'),
           ('예약이 실제로 잡혔는가',
            '「확인 후 연락드리겠습니다」가 아니라 시간이 정해지고 확인 문자가 나갔는지 '
            '보십시오. 그 차이가 통화 한 건과 두 건의 차이입니다.'),
           ('막혔을 때 무엇을 했는가',
            '못 답하는 질문에서 지어내는지, 멈추고 사람에게 넘기는지. 여기서 갈립니다.')])),
...
page('examples.html', '업종별 사례 &mdash; 예시 통화 여섯 건 | Saleringo',
     ...
     '예순다섯 업종 전체는 업종 페이지에서 하나씩 읽을 수 있습니다.',
