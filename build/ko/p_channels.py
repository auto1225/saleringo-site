...
platform = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-spot', 'crm', 'CRM',
     ...
     '일반 CRM 은 이름과 연락처를 묻습니다. 그다음은 사장님이 직접 항목을 만들어 '
     '넣으셔야 합니다. 치과의 예약 단계와 정비소의 입고 단계는 다르고, 부동산의 매물 '
     '문의와 학원의 상담 문의도 다릅니다. 그 차이를 빈칸에 채워 넣는 작업이 도입의 '
     '절반이고, 대개 거기서 멈춥니다.<br><br>여기서는 업종별로 항목과 단계를 미리 '
     '만들어 두었습니다. 치과라면 상담&rarr;검진&rarr;치료계획&rarr;시술, 정비소라면 '
     '입고&rarr;견적&rarr;승인&rarr;출고입니다. 쓰시면서 고치실 수 있지만, '
 ...
 sec('t-md sec-light bg-paper', 'who-next', '누구에게 넘기는가',
     ...
     '틀린 금액을 말하면 그 금액을 지켜야 하고, 진단 비슷한 말을 하면 그것이 기록으로 '
     '남습니다. 그래서 넘기는 자리를 업종마다 미리 정해 두고, 그 선을 넘지 않게 '
     '만들었습니다. 넘길 때는 &ldquo;확인 후 연락드리겠습니다&rdquo;로 끝내고 담당자 '
     '앞으로 건을 보냅니다 &mdash; 끊고 나서 잊히지 않습니다.',
...
voice = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-grid', 'cando', '전화로 실제로 되는 것',
     ...
     + ul([('동시에 걸려 온 전화도 받습니다',
            ...
            '전화가 지금은 그냥 끊깁니다. 그 통화들이 없어지지 않습니다.'),
           ('한국어로 말합니다',
...
kakao = '\n\n'.join([
 ...
 sec('t-md sec-dark bg-spot', 'whykakao', '왜 카카오톡인가',
     ...
     trio([('쓰시던 채널 그대로',
            ...
            '친구 수도 그대로입니다.'),
           ('답장이 밤에도 나갑니다',
            '읽음 표시만 남고 답이 다음 날 가면 그 사이에 다른 곳에 물어봅니다.'),
           ('예약 확인이 알림톡으로',
 ...
 sec('t-md sec-dark bg-grid', 'setup2', '붙이는 데 필요한 것',
     ...
     ul([('채널 연결',
         ...
         ('발송 요금', '알림톡은 건당 15원입니다. 보내신 만큼만 다음 달에 '
          '정산합니다. 받는 것은 대화 건수에 들어가고 따로 붙지 않습니다.'),
 ...
 sec('t-md sec-light bg-paper', 'limits', '안 되는 것',
     ...
     ul([('수신 동의 없는 광고 발송 &mdash; 안 합니다',
         ...
         ('심야 발송 &mdash; 막아 둡니다',
          ...
          '예약 확인처럼 요청하신 것에 대한 답은 시간과 무관하게 나갑니다.')]),
     dark=False),
